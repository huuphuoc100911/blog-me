...
@section('page-title', __('lang.admin.blog_categories.index'))
@push('styles')
    <style>
        .btn-cat-del {
            margin-left: 10px;
    ...
    <div class="content-wrapper">
                        ...
                        <div class="card h-100">
                            ...
                            <div class="card-body cat-info">
                                <h4 class="card-title text-danger">{{ $category->title }}
                                    <span onclick="changeStatusBlogCategory({{ $category->id }})"
                                        id="status-blog-category--{{ $category->id }}" style="float: right">
        ...
        @push('scripts')
            ...
            <script>
                ...
                function changeStatusBlogCategory(blogCategoryId) {
                    $.ajax({
                        url: "{{ route('admin.blog-category.change-status-blog-category') }}",
                        method: "GET",
                        data: {
                            blogCategoryId: blogCategoryId
                        },
                        success: function(data) {
                            $("#status-blog-category--" + blogCategoryId).html(data.status)
                        }
                    });
                }
            </script>
